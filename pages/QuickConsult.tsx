...
return (
    <div className="animate-fadeIn h-[calc(100vh-5rem)] bg-[#0f0f0f] flex flex-col text-white overflow-hidden font-sans">
      ...
      <div className="shrink-0 max-w-4xl mx-auto w-full px-6 pt-10 pb-4 text-center space-y-1">
        <h1 className="text-xl md:text-2xl font-light uppercase tracking-[0.4em] text-emerald-500">Strategy Lab</h1>
        <p className="text-white/40 text-[10px] tracking-[0.2em] uppercase font-medium">FIG Operational Intelligence v2.5</p>
      </div>
        ...
        <div className="max-w-4xl mx-auto w-full space-y-4 pb-8">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`w-fit rounded-2xl md:rounded-3xl px-5 py-3.5 shadow-2xl ${
                msg.role === 'user' 
                ...
                <p className="text-sm md:text-[15px] leading-relaxed whitespace-pre-wrap break-words font-light tracking-tight">{msg.text}</p>
              </div>
            </div>
          ))}
          {isLoading && (
          ...
          {activeOptions && !isCurrentlyOther && (
            <div className="flex flex-wrap gap-2 pt-2 justify-center md:justify-start">
              {activeOptions.opts.map(opt => (
                <button
                  key={opt}
                  onClick={() => handleSelection(activeOptions.field, opt, activeOptions.next, activeOptions.q)}
      ...
      <div className="shrink-0 w-full p-6 md:p-10 bg-gradient-to-t from-black via-black/80 to-transparent">
        <div className="max-w-4xl mx-auto">
          <div className={`flex items-end bg-zinc-900/90 backdrop-blur-3xl border border-white/10 rounded-[2rem] px-6 py-3 shadow-2xl transition-all duration-300 ${isChatEnabled ? 'focus-within:border-emerald-500/50 opacity-100' : 'opacity-40 grayscale pointer-events-none'}`}>
            ...
            <textarea 
              ...
              placeholder={isChatEnabled ? "Input strategic query..." : "Please select an option from the list above"}
              rows={1}
